    ...
    with c2:
        with st.expander("🗑️ Xóa hàng hóa"):
            if products:
                ...
                selected_del_label = st.selectbox("Chọn mã hàng cần xóa", options=list(del_options.keys()), index=None)
                if st.button("Xác nhận xóa"):
                    if selected_del_label:
                        ...
                        has_history = False
                        if history_data:
                            df_history = pd.DataFrame(history_data, columns=["Ngày", "Mã", "Tên Hàng Hóa", "Loại", "Số Lượng", "Ghi Chú", "Số Phiếu"] if len(history_data[0]) >= 7 else ["Ngày", "Mã", "Tên Hàng Hóa", "Loại", "Số Lượng", "Ghi Chú"])
                            has_history = del_code.upper() in df_history["Mã"].astype(str).str.strip().str.upper().values
                        
                        if has_history: st.error(f"❌ Không thể xóa mã hàng '{del_code}' vì sản phẩm này đã có lịch sử Nhập/Xuất kho!")
                        else:
                            service.delete_product(del_code)
                            st.cache_data.clear()
                            st.success(f"🎉 Đã xóa thành công hàng hóa {del_code}!"); st.rerun()
                    else: st.warning("Vui lòng chọn một hàng hóa!")
...
# --- TAB 2: NHẬP/XUẤT ---
        ...
        if st.session_state.cart:
            ...
            if st.button("✅ Xác nhận tất cả", type="primary"): 
                ...
                for _, row in edited_df_cart.iterrows():
                    # Gửi số phiếu này xuống hàm add_transaction
    ...
    if history:
        # Bổ sung dữ liệu rỗng cho các dòng lịch sử cũ (trước khi tính năng Số Phiếu được ra mắt)
        processed_history = []
        for row in history:
            row_copy = list(row)
            while len(row_copy) < 7: row_copy.append("")  # Kéo dài mảng cho đủ 7 cột
        ...
        # Đảo vị trí cột Số Phiếu lên trước Mã Hàng cho đẹp
