...
# 3. BỘ NHỚ ĐỆM (CACHE) - Dùng dạng dictionary để tránh lỗi pickle
@st.cache_data(ttl=30, show_spinner=False)
def get_cached_products(_svc):
    products = _svc.get_products()
    return [list(p) for p in products] # Chuyển tuple/object thành list
...
if menu == "Danh mục hàng hóa":
    ...
    if products:
        ...
        st.dataframe(df[["Mã", "Tên", "Đvt", "Tồn"]], width=None, hide_index=True)
        
        st.download_button(
            "📥 Xuất danh mục (.xlsx)", 
            export_to_excel(df[["Mã", "Tên", "Đvt", "Tồn"]]), 
            "DanhMuc.xlsx", 
    ...
    if hist:
        ...
        st.dataframe(df_h, width=None) # Thay thế use_container_width
        st.download_button(
            "📥 Xuất lịch sử (.xlsx)", 
            export_to_excel(df_h), 
            "LichSu.xlsx", 
            "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"
        )