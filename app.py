...
# --- GIAO DIỆN SAU KHI ĐĂNG NHẬP (SIDEBAR) ---
...
# Đảm bảo menu hiện tại hợp lệ (đề phòng đổi tài khoản quyền thấp hơn)
if st.session_state.current_menu not in menu_options:
    st.session_state.current_menu = menu_options[0]

# 2. Hiển thị Menu và bắt sự kiện thay đổi
...
# Cập nhật session_state nếu người dùng bấm chọn tab mới
...
# --- TAB 2: NHẬP/XUẤT KHO ---
    ...
    # Đã sửa lại để dùng cache cho nhanh
    ...
    if products:
        ...
        with c2: 
            note = st.selectbox("Diễn giải / Kho", options=(kho_nhap_list if trans_type == "Nhập" else kho_xuat_list), index=None)
        with c3:
            st.write("")
            st.write("")
...
# --- TAB 4: LỊCH SỬ GIAO DỊCH ---
    ...
    if history:
        ...
        AgGrid(df, gridOptions=gb.build(), theme='streamlit', height=650)
...
# --- TAB 5: QUẢN LÝ NHÂN VIÊN ---
    ...
    if employees:
        ...
        grid_response = AgGrid(df_emp, gridOptions=gb.build(), fit_columns_on_grid_load=True, theme='streamlit', update_mode=GridUpdateMode.MODEL_CHANGED, data_return_mode=DataReturnMode.FILTERED_AND_SORTED, height=400)
